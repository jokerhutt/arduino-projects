...
display(U8G2_R0); // For Arduino Nano 128x64
                  // U8G2_SH1107_PIMORONI_128X128_F_HW_I2C display(U8G2_R0);
                  // // For ESP32 128x128

// To use custom pins (software I2C), use a constructor like this instead:
// U8G2_SSD1306_128X64_NONAME_F_SW_I2C display(U8G2_R0, /* SCL=*/ A5, /* SDA=*/
// A4);
...
// To use custom pins (software I2C), Adafruit libraries usually require
// specific constructors or bit-banging libraries, but Hardware I2C is
...
inline void g_init_display() {
  ...
  display.begin();
#elif defined(USE_ADAFRUIT_SSD1306)
  // [ESP32] Pour utiliser des pins personnalisés, décommentez et modifiez la
  // ligne suivante : Wire.begin(/*SDA=*/ 21, /*SCL=*/ 22);
...
inline void g_draw_filled_round_rect(int x, int y, int w, int h, int r,
                                     int color) {
#if defined(USE_U8G2)
...
inline void g_draw_filled_triangle(int x0, int y0, int x1, int y1, int x2,
                                   int y2, int color) {
#if defined(USE_U8G2)
  display.setDrawColor(color);
